...
public class ConversationViewHolder extends StatusBaseViewHolder {
    ...
    void setupWithConversation(ConversationEntity conversation) {
        ...
        Status status = conversation.getLastStatus();
        ...
        boolean sensitive = conversation.getLastStatus().getSensitive();
        ...
        setSpoilerAndContent(ViewDataUtils.statusToViewData(status, false), listener);
    ...
    private void setConversationName(List<Account> accounts) {
        Context context = conversationNameTextView.getContext();
        String conversationName;
        if(accounts.size() == 1) {
            conversationName = context.getString(R.string.conversation_1_recipients, accounts.get(0).getDisplayName());
        } else if(accounts.size() == 2) {
            conversationName = context.getString(R.string.conversation_2_recipients, accounts.get(0).getDisplayName(), accounts.get(1).getDisplayName());
        } else {
            conversationName = context.getString(R.string.conversation_more_recipients, accounts.get(0).getDisplayName(), accounts.get(1).getDisplayName(), accounts.size() - 2);
        }

        conversationNameTextView.setText(conversationName);
    }

    private void setAvatars(List<Account> accounts) {
